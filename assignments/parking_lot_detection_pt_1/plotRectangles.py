...
if __name__ == '__main__':
     ...
     # steps of 4 because each space has four coordinates
     for i in range(0,len(spaceCoordinates),4):
         #get edges for the rectangle
         x1 = spaceCoordinates[i][0]
         y1 = spaceCoordinates[i][1]
         x2 = spaceCoordinates[i+1][0]
         y2 = spaceCoordinates[i+1][1]
         x3 = spaceCoordinates[i+2][0]
         y3 = spaceCoordinates[i+2][1]
         x4 = spaceCoordinates[i+3][0]
         y4 = spaceCoordinates[i+3][1]
         
         #draw rectangles at locations
         rectangle = cv.rectangle(img, (x1,y1), (x3,y3), (0,255,0), 1)
